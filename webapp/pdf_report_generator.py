...
- Explainability visualizations (Grad-CAM, 12-panel grid)
...
class FormalPDFReportGenerator:
    ...
    def _create_visual_report_section(self, images, metrics):
        ...
        elements.append(Paragraph("<b>C.3 AI Explainability - Grad-CAM Analysis</b>", self.styles['SubHeading']))
        
        gradcam_text = """
        Gradient-weighted Class Activation Mapping (Grad-CAM) visualizes the attention regions 
        used by the AI model for threat detection. The heatmap overlay indicates where the YOLOv8-X 
        neural network focused its attention during object detection. Red regions indicate high 
        confidence detection areas, while blue regions show lower confidence. Multi-scale analysis 
        incorporates six key components: (1) Multi-scale Gaussian filtering for feature extraction 
        across fine, medium, and coarse scales, (2) Edge importance mapping using Canny and Sobel 
        operators, (3) Superpixel-based LIME segmentation for region-wise analysis, (4) Visual 
        saliency detection via FFT spectral residual method, (5) Color distinctiveness mapping 
        through RGB distance analysis, and (6) Texture richness assessment using Laplacian operators 
        and local standard deviation. This comprehensive approach provides complete model interpretability.
        """
        elements.append(Paragraph(gradcam_text, self.styles['FormalBodyText']))
        
        if 'gradcam' in images and os.path.exists(images['gradcam']):
            try:
                img = Image(images['gradcam'], width=6*inch, height=4*inch)
                elements.append(img)
                elements.append(Paragraph(
                    "<i>Figure 2: Multi-scale Grad-CAM heatmap showing AI attention regions with threat bounding boxes</i>",
                    self.styles['MetricsText']
                ))
            except Exception as e:
                elements.append(Paragraph(f"Image loading error: {e}", self.styles['MetricsText']))
            ...
            elements.append(Paragraph(
                "Grad-CAM visualization not available for this analysis.",
                self.styles['MetricsText']
            ))
        
        elements.append(Spacer(1, 0.2*inch))
        
        # C.3: Attention Flow Visualization
        elements.append(Paragraph("<b>C.3.5 Advanced Attention Flow Analysis</b>", self.styles['SubHeading']))
        
        attention_flow_text = """
        The attention flow visualization provides a four-panel analysis of the AI model's decision-making 
        process. This advanced explainability technique breaks down the threat detection into interpretable 
        components: (1) Original Input Image - the raw underwater scene as captured by the camera, 
        (2) Multi-Scale Attention Heatmap - aggregated attention weights across multiple network layers 
        showing where the model focuses, (3) Edge-Enhanced Importance Map - highlighting structural 
        features and boundaries that contribute to detection decisions, and (4) Composite Overlay - 
        the final integrated attention map overlaid on the original image for direct correlation. 
        This multi-panel approach enables operators to understand not just what was detected, but why 
        the AI model made specific detection decisions based on visual features present in the scene.
        """
        elements.append(Paragraph(attention_flow_text, self.styles['FormalBodyText']))
        
        if 'attention_flow' in images and os.path.exists(images['attention_flow']):
            try:
                img = Image(images['attention_flow'], width=6*inch, height=4.5*inch)
                elements.append(img)
                elements.append(Paragraph(
                    "<i>Figure 3: Four-panel attention flow visualization showing AI decision-making process</i>",
                    self.styles['MetricsText']
                ))
            except Exception as e:
                elements.append(Paragraph(f"Image loading error: {e}", self.styles['MetricsText']))
            ...
            elements.append(Paragraph(
                "Attention flow visualization not available for this analysis.",
                self.styles['MetricsText']
            ))
        
        elements.append(PageBreak())
        
        # C.4: 12-Panel Enhancement Grid
        elements.append(Paragraph("<b>C.4 Comprehensive Enhancement Analysis Grid</b>", self.styles['SubHeading']))
        
        grid_text = """
        The 12-panel analysis grid provides a systematic, multi-dimensional examination of underwater 
        image enhancement performance. The grid is organized into four rows of three panels each, 
        covering: <b>Row 1 (Core Comparison):</b> Original image, Enhanced image, and Intensity change 
        map showing pixel-level modifications. <b>Row 2 (RGB Channel Analysis):</b> Red, Green, and 
        Blue channel corrections demonstrating color restoration in each spectral band - particularly 
        critical for underwater imaging where red wavelengths are severely attenuated. <b>Row 3 (Color 
        and Tone):</b> Color balance adjustments, Contrast enhancement visualization, and Brightness 
        improvement metrics. <b>Row 4 (Quality Assessment):</b> Texture preservation analysis, Edge 
        definition mapping, and comprehensive Underwater Image Quality Metrics panel displaying 
        quantitative measurements including turbidity reduction percentage, color cast correction, 
        red channel restoration, entropy gain, and information content analysis. Each panel includes 
        color-coded borders for quick reference: green for baseline, purple for channel analysis, 
        orange for tone adjustments, and red for quality metrics.
        """
        elements.append(Paragraph(grid_text, self.styles['FormalBodyText']))
        
        if 'enhancement_grid' in images and os.path.exists(images['enhancement_grid']):
            try:
                img = Image(images['enhancement_grid'], width=6.5*inch, height=4.875*inch)
                elements.append(img)
                elements.append(Paragraph(
                    "<i>Figure 4: 12-panel comprehensive enhancement analysis grid (4 rows × 3 columns)</i>",
                    self.styles['MetricsText']
                ))
            except Exception as e:
                elements.append(Paragraph(f"Image loading error: {e}", self.styles['MetricsText']))
            ...
            elements.append(Paragraph(
                "Enhancement analysis grid not available - enhancement may not have been performed.",
                self.styles['MetricsText']
            ))
